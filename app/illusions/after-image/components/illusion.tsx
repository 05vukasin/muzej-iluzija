...
export default function Illusion() {
  ...
  return (
    <div className="rounded-2xl bg-black/65 backdrop-blur-md text-white shadow-2xl ring-1 ring-white/10 p-6 sm:p-8 lg:p-10">
      ...
      {/* Wrapper bez vw — širina pune roditeljske širine, limitirana max-w-om */}
      <div className="mt-6 sm:mt-8 mx-auto w-full max-w-[520px] sm:max-w-[500px]">
        {/* KVADRAT: nema horizontalnog overflow-a na telefonu */}
        <div className="relative aspect-square rounded-xl overflow-hidden ring-1 ring-white/10 shadow-lg flex items-center justify-center bg-white">
      ...
      <div className="mt-6 space-y-5">
        {/* red 1: start/again + slider koji zauzima ostatak širine */}
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="relative flex-1 min-w-0">
            <label htmlFor="dur" className="sr-only">{t.duration}</label>
            <input
              id="dur"
              type="range"
              min={5}
        ...
        {/* red 2: izbor oblika */}
        <div>
          <span className="block text-sm sm:text-base text-white/90">{t.shape}</span>
...
function Silhouette({
  ...
  // MANJE siluete: 40% kvadrata
  const size = "40%";

  if (shape === "circle") {
    return <div aria-hidden className="bg-black" style={{ width: size, height: size, borderRadius: "9999px" }} />;
  }
  if (shape === "square") {
    return <div aria-hidden className="bg-black" style={{ width: size, height: size }} />;
  }
  if (shape === "cross") {
    return (
      <div aria-hidden className="relative" style={{ width: size, height: size }}>
        <div className="absolute inset-0 m-auto bg-black" style={{ width: "18%", height: "100%" }} />
        <div className="absolute inset-0 m-auto bg-black" style={{ width: "100%", height: "18%" }} />
      </div>
    );
  }
  return (
    <svg aria-hidden viewBox="0 0 100 100" style={{ width: size, height: size, display: "block" }}>
      <path d="M50 86 L16 52 A20 20 0 0 1 50 22 A20 20 0 0 1 84 52 Z" fill="black" />
    </svg>
  );
}
